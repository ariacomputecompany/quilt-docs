...
- `name` is required.
- `image` uses Quilt's native container selector model by default. Common selectors include `prod`, `prod-gui`, and curated runtime selectors such as `nightly/<profile>`.
