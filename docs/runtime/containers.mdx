...
- `name` is required.
- `image` uses Quilt's native container selector model by default. Common selectors include `prod`, `prod-gui`, and curated runtime selectors such as `rlm`, `aegis`, and `amp`.
...
- Curated runtimes such as RLM, Aegis, and AMP are selected through the same native `image` field rather than through a separate launcher product.
...
If your container is already serving HTTP or WebSocket traffic and you need an external URL, use Quilt's published-service surface instead of bolting on your own proxy contract.

See [Published Services](/access/published-services).
...
GPU requests belong in the create payload using `gpu_count` and optionally `gpu_ids`. GPU-backed containers run on compatible user-owned execution nodes, not on a cloud GPU host. Quilt validates deterministic GPU request failures before execution begins.

Possible deterministic failures:

- `400` for invalid GPU shapes
- `400` for invalid `strict` plus GPU combinations
